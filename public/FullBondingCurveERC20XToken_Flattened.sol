...
// File BSC_HAVEN/FullBondingCurveERC20XToken.sol
...
/**
 * @title FullBondingCurveERC20XToken
 * @notice Bonding curve with X Token as trading pair (pre and post bonding)
 * @dev All fees collected in X Token, auto-swapped to ETH at percentage-based thresholds
 */
contract FullBondingCurveERC20XToken is ERC20, Ownable, ReentrancyGuard {
    ...
    uint256 public preGradFactoryFee = 50;   // 0.5% factory (50 basis points)
    uint256 public preGradCreatorFee = 50;   // 0.5% creator (50 basis points)
    uint256 public postGradFactoryFee = 20;  // 0.2% factory (20 basis points)
    uint256 public postGradCreatorFee = 20;  // 0.2% creator (20 basis points)
    ...
    /**
     * @notice Buy tokens with X Token (1% fee in X Token)
     */
    function buy(uint256 xTokenAmount, uint256 minTokensOut) external nonReentrant {
        _buyFor(msg.sender, xTokenAmount, minTokensOut);
    }

    /**
     * @notice Buy tokens for a specific recipient (used by factory for initial buy)
     */
    function buyFor(address recipient, uint256 xTokenAmount, uint256 minTokensOut) external nonReentrant {
        require(msg.sender == factory, "Only factory can buy for others");
        _buyFor(recipient, xTokenAmount, minTokensOut);
    }

    /**
     * @notice Internal buy logic
     */
    function _buyFor(address recipient, uint256 xTokenAmount, uint256 minTokensOut) internal {
        require(!isGraduated, "Token graduated");
        require(xTokenAmount > 0, "No X Tokens specified");
        require(recipient != address(0), "Invalid recipient");

        // Calculate pre-grad fees (split between factory and creator)
        uint256 totalPreGradFee = preGradFactoryFee + preGradCreatorFee;
        uint256 feeXToken = (xTokenAmount * totalPreGradFee) / FEE_DENOMINATOR;
        uint256 factoryFeeXToken = (xTokenAmount * preGradFactoryFee) / FEE_DENOMINATOR;
        uint256 creatorFeeXToken = feeXToken - factoryFeeXToken;
        uint256 xTokenAfterFee = xTokenAmount - feeXToken;

        // Transfer X Tokens from user (total amount including fee)
        IERC20(X_TOKEN_ADDRESS).safeTransferFrom(msg.sender, address(this), xTokenAmount);
        ...
        emit Buy(recipient, xTokenAmount, tokensOut, feeXToken);
    ...
    /**
     * @notice Sell tokens for X Token (split fee: factory + creator)
     */
    function sell(uint256 tokenAmount, uint256 minXTokenOut) external nonReentrant {
        ...
        // Send X Tokens to user (after fee)
    ...
    function _autoSwapProjectTokenFeesToETH() internal {
        ...
        // Swap path: PROJECT Token → X Token → WETH
    ...
    function setFee(uint8 feeType, uint256 newFeeBps) external {
        ...
        if (feeType == 0) {
            preGradFactoryFee = newFeeBps;  // Pre-grad factory fee
        } else if (feeType == 1) {
            preGradCreatorFee = newFeeBps;  // Pre-grad creator fee
        } else if (feeType == 2) {
            postGradFactoryFee = newFeeBps; // Post-grad factory fee
        } else if (feeType == 3) {
            postGradCreatorFee = newFeeBps; // Post-grad creator fee
        }
